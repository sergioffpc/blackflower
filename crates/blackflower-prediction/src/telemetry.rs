...
impl ReconciliationObservation {
        ...
        E: StdError + 'static,
    {
        ...
        #[cfg(feature = "metrics")]
        {
            ...
            .increment(1);
            metrics::histogram!("blackflower_prediction_reconciliation_duration_seconds")
                .record(self.started.elapsed().as_secs_f64());
            if let Ok(ReconciliationOutcome::Reconciled {
                resimulated_ticks, ..
            }) = _result
            {
                metrics::histogram!("blackflower_prediction_resimulated_ticks")
...
pub(crate) fn describe_metrics() {
    #[cfg(feature = "metrics")]
    {
        ...
        metrics::describe_histogram!(
            "blackflower_prediction_reconciliation_duration_seconds",
            Unit::Seconds,
            "Wall-clock duration of authoritative prediction reconciliation",
        );
        metrics::describe_histogram!(
            "blackflower_prediction_resimulated_ticks",
            Unit::Count,
