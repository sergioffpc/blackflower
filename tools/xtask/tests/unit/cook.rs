...
fn cooks_texture_for_runtime_capability_selection() -> anyhow::Result<()> {
    ...
    let texture = blackflower_rendering_textures::TextureAsset::from_bytes(store.read_asset(&id)?)?;
    ...
    let upload =
        texture.transcode(blackflower_rendering_textures::TextureTargetCapabilities::default())?;
    assert_eq!(
        upload.format,
...
#[allow(
    clippy::too_many_lines,
    reason = "the acoustic asset graph test keeps all manifests and dependency assertions together"
)]
...
"#,
    ...
    let material_asset =
        blackflower_acoustics::AcousticMaterialLibrary::from_bytes(&store.read_asset(&materials)?)?;
    let prefab_asset =
...
fn watcher_debounces_a_recook_and_publishes_the_new_snapshot() -> anyhow::Result<()> {
    ...
    loop {
        ...
        match event {
            AssetWatchEvent::Reloaded(reload) if reload.status() == AssetReloadStatus::Reloaded => {
                let asset = AssetId::from_str("fixtures/example")?;
                assert_eq!(reload.snapshot().generation().get(), 1);
                assert_eq!(
...
fn corrupt_cache_objects_are_rebuilt() -> anyhow::Result<()> {
    ...
    let store = fixture.open_store()?;
    let id = AssetId::from_str("fixtures/example")?;
    let resolved = store.resolve(&id).context("missing cooked asset")?;
    let record = resolved.record();
    let content_hash = record.content_hash;
    let recipe_hash = record.recipe_hash;
    let recipe_path = fixture
        .target
        .join("asset-cache/recipes")
        .join(format!("{recipe_hash}.toml"));
    let recipe_text = fs::read_to_string(&recipe_path)?;
    let _recipe: toml::Table = toml::from_str(&recipe_text)?;
    assert!(!recipe_text.trim_start().starts_with('{'));
    assert!(recipe_path.is_file());
    assert!(!recipe_path.with_extension("json").exists());
    let cache = fixture
        .target
...
fn exr_texture() -> anyhow::Result<Vec<u8>> {
    ...
    let mut output = std::io::Cursor::new(Vec::new());
    image::DynamicImage::ImageRgba32F(image).write_to(&mut output, image::ImageFormat::OpenExr)?;
    Ok(output.into_inner())
}
