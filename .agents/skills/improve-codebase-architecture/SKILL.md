...
-   Call the Skill tool with "codebase-design" for the architecture vocabulary
    ...
    **locality**) and its principles (the deletion test, "the interface is the
    ...
    terms exactly in every suggestion, and don't drift into "component,"
...
recently changed. Decide _where_ to look before you look:
...
-   Otherwise, walk back a good stretch of the commit history
    (`git log --oneline`) to find the codebase's hot spots, the files and areas
...
in the card (e.g. a warning callout: _"contradicts ADR-0007, but worth reopening
...
Once the user picks a candidate, call the Skill tool with "grilling" to walk the
...
"domain-modeling" to keep the domain model current as you go:
...
-   **User rejects the candidate with a load-bearing reason?** Offer an ADR,
    ...
    don't re-suggest it?"_ Only offer when the reason would actually be needed
    by a future explorer to avoid re-suggesting the same thing; skip ephemeral
    reasons ("not worth it right now") and self-evident ones.
-   **Want to explore alternative interfaces for the deepened module?** Call the
    Skill tool with "codebase-design" and use its design-it-twice parallel
    sub-agent pattern.
