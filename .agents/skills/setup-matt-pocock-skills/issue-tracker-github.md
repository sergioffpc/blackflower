...
-   **Child ticket**: an issue linked to the map as a GitHub sub-issue (`gh api`
    ...
    child body. Labels: `wayfinder:<type>`
...
-   **Blocking**: GitHub's **native issue dependencies**, the canonical,
    UI-visible representation. Add an edge with the dependencies endpoint:
...
-   **Frontier query**: list the map's open children
    ...
    drop any with an open blocker (`issue_dependencies_summary.blocked_by > 0`,
    or an open issue in the `Blocked by` line) or an assignee; first in map
    order wins.
