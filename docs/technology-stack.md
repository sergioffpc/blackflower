...
The two runtime implementations retain C++23, Clang 21, CMake, Ninja, sccache, vcpkg, GoogleTest, Google Benchmark, and the repository's analysis/sanitizer workflow. The owner selected Python for the offline cooker; its interpreter baseline, dependency management, packaging, and signing backend remain to be specified. Prefer standard-library facilities and Boost for remaining gaps; the owner's explicit library choices above take precedence over a default library preference.
...
The MVP also requires an [offline cooker and two signed content packs per scenario](cooker-and-packs.md). The Python cooker runs on Linux and produces separate client (Prediction/Presentation) and server (Simulation) packs. Each runtime verifies its own pack, expected role, integrity, and compatibility before creating worlds; admission compares the proposed common scenario build identity. SHA-256 and Ed25519 remain algorithm proposals, with OpenSSL EVP proposed for the C++ loader; the Python signing backend remains open. No new package pin is selected. Shader and physics target artifacts must be prepared offline and loaded without runtime asset cooking.
...
The [cooker design](cooker-and-packs.md#selected-cooker-stack-and-model-processing) defines import, validation, optimization, encoding, and signed packaging. Binding/FFI choice and compatible Python/native version pins remain open. The proposed MVP optimization policy preserves geometry and attributes; lossy simplification is not implicitly selected.

The owner requires [Vulkan-only graphics](adr/0006-use-vulkan-only.md). Validate that the client build and delivered dependencies exclude the DirectX 12 backend and that runtime execution uses Vulkan. Backend choice is settled; exact capability profiles and precompiled SPIR-V integration remain open.
...
2. Execute it on the P620 and render a primitive using cooker-produced SPIR-V through the required Vulkan path without runtime source compilation with the pinned Falcor dependency set. Record the driver, SDK, runtime files, and graphics backend actually used.
...
The [repository layout proposal](repository-layout.md) keeps Python authoring tools independent of C++ runtime builds, with shared format specifications and conformance fixtures.
