...
on Linux produces two signed packs per scenario, one for each runtime role.
Their libraries execute inside those applications. See the
...
is independently signed and loaded only by its intended runtime; matching signed
scenario build identities establish proposed admission compatibility. The
