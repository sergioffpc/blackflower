...
The owner requires an authoritative Linux server and Windows clients cross-compiled in Linux for the MVP. Its reference deployment is specified in section 7. The owner selected Falcor, PhysX, Slang, Flecs, GameNetworkingSockets, and Steam Audio; the [technology stack](technology-stack.md) records their proposed placement and compatibility work. The owner also requires offline-cooked content delivered in a verified signed pack from the MVP onward; see the [cooker proposal](cooker-and-packs.md). A Falcor exception adapter and the audio device backend remain proposals. Deployment and fidelity requirements for the broader training platform remain open.
...
The proposed [C4 container view](c4.md#c2-containers), [server components](c4.md#c3-authoritative-server-components), and [client components](c4.md#c3-windows-client-components) describe the next software structure. Runtime libraries run inside the client and server executable types. The [offline cooker and pack loader](cooker-and-packs.md) add a development executable and external startup verification, not another ECS world. The Simulation World interface and client prediction/reconciliation interface are the principal behavior-test surfaces. Input-source equivalence and presentation isolation are checked through those interfaces; running-client/server and perceptual checks cover the actual integrations.
...
The MVP requires a signed cooked-content pack alongside the runtime binaries, and an independently provisioned trusted public key for verification. Private content-signing keys stay in the packaging environment. No MVP deployment has been validated. Record actual OS builds, drivers, build configuration, display/audio settings, and network conditions with the first delivery evidence, and link operational instructions when introduced.
...
| Offline cooking; verified signed content pack required before gameplay | [ADR-0005](adr/0005-require-signed-cooked-content.md), required by the owner; format, algorithms, and integration remain proposed. |
...
| ARCH-Q08 | Start from the signed cooked pack without source assets; repeat with tampering, malformed metadata, unknown signing key, incompatible content, and a different peer pack. | Valid compatible content initializes on both targets; invalid content is rejected before world initialization, and a peer content mismatch is rejected before admission. No asset cooking or pack I/O executes inside ECS. |
...
| R-010 | Offline preparation and loading of signed packs have not been implemented, including Falcor precompiled shaders and target-specific GPU physics artifacts. | A runtime importer/compiler or incomplete signature coverage could violate the required deployment model. | Prove the end-to-end cooker/loader path, specify canonical authenticated metadata and limits, provision separate content trust keys, and test invalid-pack rejection before the first slice. |
