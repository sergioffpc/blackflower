...
two independently verified signed packs per scenario from the MVP onward:
simulation for Simulation/Prediction and presentation for Presentation, with
Slang compiling client shaders to SPIR-V offline; see the
...
| [Content module](../src/modules/content/content.h)                   | Own pack bytes and authenticate their role, manifest and payload before returning validated scene values.                           |
...
The content loader accepts simulation or presentation resources independently of
host platform. Resource schemas define representation compatibility. It
separates layout decoding, authentication and resource validation internally
while retaining one public loading contract. Scene decoding checks each field as
it consumes the encoded collections and propagates typed errors without
returning partial scenes. Geometry and gameplay suitability belong to consumers;
the loader imposes no reference-scenario dimensions, quantities or placement
rules.
...
geometry, derives the common build identity, and writes two separately signed
artifacts in private staging. It reopens and verifies both completed files
before publishing the pair directory. The C++ content harness maps one file
read-only, verifies trusted-key authentication and scene encoding, then reports
...
scenario build identity, since simulation and presentation artifact identities
differ. Cooking and signing occur offline; the
[pack design](cooker-and-packs.md) records the unimplemented details. The
[runtime lifecycle proposal](runtime-lifecycle.md) defines readiness gates,
per-peer admission/removal, one client connection deadline through
usable-baseline preparation, focus behavior, and cleanup after partial startup
or in-flight work. These application states surround the existing ECS phases and
remain proposals.
...
The server requires the simulation pack. Clients require both simulation and
presentation packs, with the same simulation artifact shared across deployments.
Each runtime receives an independently provisioned trusted public key for
...
| Two C++ runtimes and one offline Python cooker using Assimp, meshoptimizer, and Slang-to-SPIR-V; two verified signed content packs per scenario required before gameplay | [ADR-0005](adr/0005-require-signed-cooked-content.md), required by the owner; primitive format and algorithms are implemented in ADR-0007; mesh/shader/SDK integration remains pending.                                          |
...
[ADR-0010](adr/0010-platform-independent-content-roles.md) defines the
platform-independent simulation/presentation split and the pack v1 format.
...
| ID          | Stimulus and operating conditions                                                                                                                    | Expected response and threshold                                                                                                                                  |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CONTENT-Q01 | Cook an OpenUSD scene with dimensions and collection counts different from the reference; load each role alone with independently provisioned trust. | Both artifacts preserve authored geometry, light parameters and spawn positions without scenario-specific validation; PackIds differ and ScenarioBuildIds match. |
...
| R-010 | Primitive OpenUSD cooking and signed loading are implemented; remaining integration includes Falcor Vulkan loading of offline Slang-compiled SPIR-V, content-role compatibility, and runtime preparation of GPU physics resources. | A runtime importer/compiler or incomplete signature coverage could violate the required deployment model.                                       | Extend the implemented bounded path with Assimp/meshoptimizer and native SDK artifacts, provision production content trust, and test gameplay admission against the common scenario build identity. Retain runtime validation and the current minimal functional test coverage. |
