...
converting lengths, preserving signature and scene validation. It retains a
complete file and signature transcript in memory; see
[ADR-0009](adr/0009-typed-errors-and-content-size-policy.md).
...
| OpenUSD scene input, uv-managed Python, primitive pack v1, cryptography/libsodium and independent trust                                                          | [ADR-0007](adr/0007-minimal-pack-format-and-trust.md), accepted for #21.                                                                                                                                                         |
...
| ID          | Stimulus and operating conditions                                                                                          | Expected response and threshold                                                                                                                                             |
| ----------- | -------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CONTENT-Q01 | Cook the reference OpenUSD scene on Linux; load each role alone with independently provisioned trust on Linux and Windows. | Both valid artifacts expose exactly the 20,000 mm interior, 1,800 by 600 mm capsule, two blocks and four canonical spawn values; PackIds differ and ScenarioBuildIds match. |
...
R-012: Content ingestion retains the file and signature transcript in memory
without a policy byte cap. Memory use grows with input size and allocation
failure is not recoverable in the current no-exceptions build. Measure
representative content before selecting streaming or another loading strategy;
do not introduce an unsubstantiated replacement cap.
