...
ClientScene alternative of the returned variant. Resource schemas define
representation compatibility. It separates layout decoding, authentication and
resource validation internally while retaining one public loading contract.
Scene collision shapes describe collision shapes independently of visual assets.
Decoding checks each field as it consumes the encoded collections and propagates
typed errors without returning partial scenes. Geometry and gameplay suitability
belong to consumers; the loader imposes no reference-scenario dimensions,
quantities or placement rules.
...
scenario build identity, since consumer artifact identities may differ. Cooking
...
`.bfagent`, and human clients select `.bfclient`. Server and agent content
currently share collision geometry; only server content contains spawn points
and only client content contains lights. The autonomous participant runtime and
model-driven control remain outside the content preparation scope. The loader
receives a path and trusted keys without a role selector. Each runtime receives
an independently provisioned trusted public key for verification. Private
