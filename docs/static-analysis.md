...
C/C++ analysis uses a manual Release build with the project's Clang 21, C++23, CMake presets, and pinned vcpkg baseline. Dependencies are configured before CodeQL initialization; CodeQL then observes compilation of the project targets, including tests and benchmarks. Compiler caching is disabled in this job so cache hits cannot hide translation units from extraction. GitHub Actions analysis uses no build. The separate Linux CI remains responsible for clang-tidy, formatting, tests, and sanitizers.
